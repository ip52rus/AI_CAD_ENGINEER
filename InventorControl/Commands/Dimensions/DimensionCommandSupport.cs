...
internal static class DimensionCommandSupport
{
        ...
        DrawingView view)
    {
        ...
        List<DrawingCurve> curves = new();

        DrawingCurvesEnumerator enumerator =
            view.DrawingCurves[Type.Missing];

        foreach (DrawingCurve curve in enumerator)
        {
            curves.Add(curve);
        }
    ...
    public static bool TryParsePointIntent(
        string value,
    ...
    public static bool TryParseDimensionType(
        string value,
    ...
    public static object? PointToObject(Point2d? point)
    {
        if (point == null)
        {
            return null;
