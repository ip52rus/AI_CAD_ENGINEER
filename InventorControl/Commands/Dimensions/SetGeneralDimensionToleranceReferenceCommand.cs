...
public sealed class SetGeneralDimensionToleranceReferenceCommand : IInventorCommand
{
    private readonly Inventor.Application _inventor;

    public SetGeneralDimensionToleranceReferenceCommand(Inventor.Application inventor)
    {
    ...
    public string Execute(JsonElement root)
    {
        ...
        {
            diagnostics.Add(new { scope = "Tolerance.SetToReference", message = exception.Message, exceptionType = exception.GetType().FullName });
            return GeneralDimensionToleranceSupport.CreateError(
                "Failed to set general dimension tolerance to reference.",
                diagnostics);
        }

        return GeneralDimensionToleranceSupport.CreateSuccess(new
        {
            capability = "set_general_dimension_tolerance_reference",
