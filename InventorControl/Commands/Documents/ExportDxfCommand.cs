...
public sealed class ExportDxfCommand : IInventorCommand
{
    private const string DxfTranslatorClientId =
        "{C24E3AC4-122E-11D5-8E91-0010B541CD80}";

    private readonly Inventor.Application _inventor;

    public ExportDxfCommand(Inventor.Application inventor)
    {
    ...
    public string Execute(JsonElement root)
    {
        ...
        {
            return CreateError("Invalid DXF export outputPath.", diagnostics);
        }

        if (!TryGetIniPath(root, diagnostics, out string iniPath))
        {
            return CreateError("Invalid DXF export iniPath.", diagnostics);
        }
        ...
        if (fileExistedBefore)
        {
            ...
            if (!overwrite)
            {
                diagnostics.Add(new
                {
                    ...
                    message = "Destination DXF already exists and overwrite is false.",
                    outputPath
                });

                return CreateError("Destination DXF already exists.", diagnostics);
            }
        }

        if (!TryGetDxfTranslator(out TranslatorAddIn? translator, diagnostics))
        {
            return CreateError("DXF translator add-in is not available.", diagnostics);
        }
        ...
        if (!outputFile.Exists)
        {
            ...
            {
                scope = "output.file",
                message = "Expected DXF file does not exist after SaveCopyAs.",
                outputPath
            });
        ...
        out string outputPath)
    {
                ...
                StringComparison.OrdinalIgnoreCase))
        {
            ...
            {
                scope = "input.outputPath",
                message = "outputPath extension must be .dxf.",
                outputPath
            });
        ...
        out string iniPath)
    {
        ...
        if (!System.IO.File.Exists(iniPath))
        {
            ...
            {
                scope = "input.iniPath",
                message = "DXF export INI file does not exist.",
                iniPath
            });
    ...
    private bool TryGetDxfTranslator(
        ...
        List<object> diagnostics)
    {
        ...
        try
        {
            ApplicationAddIn addIn =
                _inventor.ApplicationAddIns.ItemById[DxfTranslatorClientId];

            diagnostics.Add(new
            {
                scope = "ApplicationAddIns.ItemById",
                message = "DXF Translator Add-In resolved.",
            ...
            if (!dxfTranslator.SupportsSaveCopyAs)
            {
                diagnostics.Add(new
                {
                    scope = "TranslatorAddIn.SupportsSaveCopyAs",
                    message = "DXF translator does not support SaveCopyAs."
                });

                return false;
            }

            if (!dxfTranslator.TranslatorAvailable)
            {
                diagnostics.Add(new
                {
                    scope = "TranslatorAddIn.TranslatorAvailable",
                    message = "DXF translator is not available."
                });
            ...
            diagnostics.Add(new
            {
                ...
                message = "DXF translator is usable.",
                supportsSaveCopyAs = dxfTranslator.SupportsSaveCopyAs,
                translatorAvailable = dxfTranslator.TranslatorAvailable,
                fileExtensions = SafeRead(() => dxfTranslator.FileExtensions),
                supportsSaveCopyAsFrom = SafeRead(() => dxfTranslator.SupportsSaveCopyAsFrom)
            });
        ...
        catch (Exception exception)
        {
            diagnostics.Add(new
            {
                ...
                message = exception.Message,
                exceptionType = exception.GetType().FullName,
                clientId = DxfTranslatorClientId
            });
