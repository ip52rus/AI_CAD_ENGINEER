...
| Document operations | PARTIAL | `get_open_documents`, `open_document`, `activate_document`, `update_active_document`, `save_document`, `save_document_as`, `close_document`, `create_drawing_document`, `export_pdf`, `export_dwg`, `export_dxf` | `create_drawing_document`, `save_document`, `save_document_as`, `export_pdf`, `export_dwg`, `export_dxf` | - | print workflow; `close_document` modal semantics are unaudited/deferred | P1 |
...
| Drawing views | PARTIAL | `get_drawing_views`, `get_drawing_view`, `get_drawing_views_detailed`, `get_drawing_view_relationships`, `get_drawing_curves`, `get_view_model_references`, `get_curve_model_reference`, `create_base_view`, `create_projected_view`, `create_auxiliary_view`, `add_drawing_view_break`, `move_drawing_view`, `move_drawing_view_annotation_text`, `delete_drawing_view`, `rename_drawing_view`, `rotate_drawing_view`, `set_drawing_view_scale`, `set_drawing_view_style`, `set_drawing_view_label_visibility`, `set_drawing_view_scale_inheritance`, `set_drawing_view_alignment`, `set_drawing_view_suppressed` | drawing views / relationships / curve geometry, `get_drawing_curves` with factual `DrawingCurve.EdgeType` and optional `startIndex`/`count` range support, `create_auxiliary_view`, `add_drawing_view_break`, and Package 65A detail/source view annotation text movement are verified areas | - | detail fence movement and crop view improvements are not covered by confirmed commands | P1 |
...
| Drawing Export Hands | VERIFIED | `export_pdf`, `export_dwg`, `export_dxf` | `export_pdf`, `export_dwg`, `export_dxf`; Package 63A verifies explicit save/export paths use scoped `Application.SilentOperation`, restore previous state, avoid modal Save dialogs, and leave referenced source IPT plus unrelated documents unsaved/clean | - | print workflow is not implemented | P0 maintained |
...
| Drawing layout map Eye | VERIFIED | `get_drawing_layout_map` | Package 64A verified normalized factual sheet-space map for external global layout reasoning: sheet geometry, reserved areas, views, dimensions, notes, tables, center annotations, symbols, isolated diagnostics; Package 65A extends drawing views with `DrawingView.ViewAnnotation` and detail-definition facts. Fixture detail view `РЕЦЕСС` returned primary `TextPosition=(8,13.4)`, `Text="РЕЦЕСС"`, source view `ВИД1`, `DisplayDefinitionInBase=true`, circular fence center `(8,10.7)`, and fence radius `2.2` | - | hole/thread note leader geometry not always exposed; dimension parent-view linkage partial; view label bounds not reliable; some symbol subtypes may lack bounds; `DrawingViewAnnotation.RangeBox` and referenceKey may be unavailable | P0 maintained |
| Detail view annotation text control | VERIFIED | `get_drawing_layout_map`, `move_drawing_view_annotation_text` | Package 65A verified `DrawingViewAnnotation.TextPosition` movement on disposable `вал тестовый_E2E_ESKD_02.idw` copy: detail view `РЕЦЕСС` primary text moved `(8,13.4) -> (5,13.4)` with actual `(5,13.4)`, `positionNormalizedByInventor=false`, visual preview confirmed source-side label movement, restore to `(8,13.4)` passed, and detail fence/view/source IPT dirty state were preserved | - | no automatic text-slot selection; no fence movement; `SecondTextPosition` may be unavailable depending on Inventor object state | P0 maintained |
...
| Model feature Eyes | VERIFIED | `get_model_feature_tree`, `get_hole_features`, `get_thread_features`, `get_feature_details`, `get_model_parameters` | `get_model_feature_tree`, `get_model_parameters`, `get_hole_features`, `get_thread_features`, and `get_feature_details` live-verified for active `PartDocument` during first ESKD end-to-end test; existing DrawingDocument referenced-model paths remain supported | - | write commands for model features are not part of confirmed Runtime scope | P0 maintained |
| Model geometry Eyes | VERIFIED | `get_surface_bodies`, `get_body_faces`, `get_face_edges` | `get_surface_bodies`, `get_body_faces`, and `get_face_edges` live-verified for active `PartDocument`; existing DrawingDocument referenced-model paths remain supported | - | no missing items confirmed by Package 13-17 audits | P0 maintained |
| Sketch / constraint / work feature Eyes | VERIFIED | `get_sketches`, `get_sketch_geometry`, `get_sketch_constraints`, `get_sketch_dimensions`, `get_work_features` | `get_sketches`, `get_sketch_geometry`, `get_sketch_constraints`, `get_sketch_dimensions`, and `get_work_features` live-verified for active `PartDocument`; existing DrawingDocument referenced-model paths remain supported; `get_sketches` preserves pre-existing Inventor edit state and derives `isActive` from `Application.ActiveEditObject` identity, not `PlanarSketch.Edit()` | - | model/sketch constraint writes are outside Package 15 audit scope | P0 maintained |
