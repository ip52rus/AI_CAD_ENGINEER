...
| Custom Tables metadata discovery | VERIFIED | `get_drawing_table_collections` | confirmed GOST `Список деталей` is `Sheet.CustomTables` / `kCustomTableObject` | - | detailed row/cell reading is separate | P0 maintained |
| Custom Tables detailed rows/cells | MISSING | - | - | - | typed detailed Eye for `Sheet.CustomTables` columns, rows, and cells | P0 next audit |
...
| Document properties | PARTIAL | `get_document_properties`, `get_document_property`, `get_document_property_by_id`, `get_document_property_sets`, `set_document_property`, `set_document_property_by_id` | not separately recorded | - | no missing items confirmed by Package 13-16 audits | P2 |
...
| Model geometry Eyes | VERIFIED | `get_surface_bodies`, `get_body_faces`, `get_face_edges` | surface bodies, faces, edges recorded as verified areas; exact PASS command list not recorded | - | no missing items confirmed by Package 13-16 audits | P0 maintained |
...
| Diagnostics / metadata | PARTIAL | `get_application_addins`, `get_gost_metadata` | not separately recorded | - | no missing items confirmed by Package 13-16 audits | P2 |
...
Package 16 confirmed that a visible GOST `Список деталей` table in `Sborka1.idw` is exposed by Inventor as:
...
It is not `Sheet.PartsLists`.

## Priority notes

- P0: run the next audit for a typed `Sheet.CustomTables` Eye.
...
Do not add automatic scenarios such as auto-placement, collision resolution, best-view selection, dimension selection, BOM generation, or complete drawing generation into the C# Runtime.
