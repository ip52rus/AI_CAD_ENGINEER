...
| Drawing layout map Eye | VERIFIED | `get_drawing_layout_map` | Package 64A verified normalized factual sheet-space map for external global layout reasoning: sheet geometry, reserved areas, views, dimensions, notes, tables, center annotations, symbols, isolated diagnostics; fixture returned 4 views, 6 dimensions, 9 general notes, 2 hole/thread notes, 3 sketched symbols; border bounds `0,0 -> 42,29.7`; title block bounds `23,0 -> 42,6`; 6/6 dimensions had text bounds, dimension-line geometry, and both extension-line geometries; stable reference keys verified for views, dimensions, notes, hole/thread notes, and sketched symbols | - | hole/thread note leader geometry not always exposed; dimension parent-view linkage partial; view label bounds not reliable; some symbol subtypes may lack bounds | P0 maintained |
